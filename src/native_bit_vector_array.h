...
public:
    ...
    bitArraySet(const u_int64_t nb_elements, const int nb_bit_per_element) : _nb_elements(nb_elements), _nb_bit_per_element(nb_bit_per_element),  _nb_bit_per_unit(64)
    {
...
//        cout<<"_get_starting_position_in_the_unit "<<_nb_unit<<" for "<<_nb_elements<<" each of size "<<_nb_bit_per_element<<endl;
        create();
//        cout << "_nb_unit = "<<_nb_unit<<endl;
    ...
    void createTable(){
        _table = (u_int64_t *)malloc(sizeof(u_int64_t)*_nb_unit);
        if(_table == NULL){
    ...
    void set_i(const u_int64_t indice_element, u_int64_t to_set){
...
//        cout<<"excluded_ending_position_in_the_unit "<<excluded_ending_position_in_the_unit<<endl;
        if (excluded_ending_position_in_the_unit<=_nb_bit_per_unit) {
            ...
            // 4/ res = res | shifted
            const u_int64_t eraser_mask = ~(get_mask(_nb_bit_per_element)<<(_nb_bit_per_unit-excluded_ending_position_in_the_unit));
...
//            cout<<"table1avt "<<_table[starting_unit_indice]<<endl;
            ...
            u_int64_t eraser_mask = _mask_unit<<(_nb_bit_per_unit-starting_position_in_the_unit);
...
//            cout<<"table2 "<<_table[starting_unit_indice+1]<<endl;
    ...
    u_int64_t get_i(const u_int64_t indice_element)const{
        const u_int64_t starting_unit_indice = _get_starting_unit_indice(indice_element);
        const int starting_position_in_the_unit = _get_starting_position_in_the_unit(indice_element, starting_unit_indice);
...
//        cout<<"res table2 "<<_table[starting_unit_indice+1]<<endl;
        ...
        const int excluded_ending_position_in_the_next_unit = excluded_ending_position_in_the_unit - _nb_bit_per_unit;
        // excluded_ending_position_in_the_next_unit = 11-8=3
        u_int64_t result=get_mask(_nb_bit_per_unit-starting_position_in_the_unit);           // set a mask for the last bits present in the starting unit
        ...
        u_int64_t starting_bits_of_next_unit =                                               // get the first bits of the next unit that correspond to the end of the element.
...
//        cout<<"starting_bits_of_next_unit "<<starting_bits_of_next_unit<<endl;
	...
	void save(std::ostream& os) const
	{
		...
		os.write(reinterpret_cast<char const*>(_table), sizeof(u_int64_t)*_nb_unit);
	...
	void load(std::istream& is)
	{
		...
		create();
		is.read(reinterpret_cast<char*>(_table), sizeof(u_int64_t)*_nb_unit);
	}
...
private:
     ...
     */
    inline u_int64_t _get_starting_unit_indice(const u_int64_t indice_element)const{
        return ((indice_element*_nb_bit_per_element)/_nb_bit_per_unit);
    }
    
    
    inline int _get_starting_position_in_the_unit(const u_int64_t element_indice, const u_int64_t starting_unit_indice)const{
        return element_indice*_nb_bit_per_element - starting_unit_indice*_nb_bit_per_unit;
    }
    
    u_int64_t get_mask(const int nb_bit)const{
        u_int64_t res=~0;
    ...
    u_int64_t   _nb_unit;      // an unit = an entry of the array. A unit stores 1 or more element(s)
    u_int64_t * _table;
    u_int64_t   _mask_unit;
    u_int64_t   _mask_element;
    
    u_int64_t   _nb_elements;     // an element is a stored stuff whose size is <= _nb_bit_per_unit
